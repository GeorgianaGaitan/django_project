...
def test_build_dict():
    ...
    assert build_dict([]) =={"odd":[],
                             "even":[]}
    assert build_dict([-1]) == {"odd":[-1],
                             "even":[]}
...
def test_is_adult():
    dog1 = Dog.from_string("Shadow-4-2")
    dog2 = Dog.from_string("Shadow-4-2")
    dog3 = Dog.from_string("Shadow-4-2")
    assert Dog.is_adult(dog1.age) == False
    assert Dog.is_adult(dog2.age) == False
    assert Dog.is_adult(dog3.age) == False
    #assert dog1.to_string() ==  "This dog's name is {name}, it has {legs} legs, and is of {age} years of age.".format(name="Shadow",
#legs=4, age=2)
